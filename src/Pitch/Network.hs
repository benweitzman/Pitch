...
data Request = Request { rtype :: RequestType, path :: BS.ByteString, headers :: [(BS.ByteString, BS.ByteString)], body :: BS.ByteString }

data Response = Response { version :: String, statuscode :: Int, contentsize :: Int }
 
instance Show Request where
  show r = "Request {" ++ show (rtype r) ++ " " ++ (BS.unpack $ path r) ++ foldl (\acc (k, v) -> acc ++ "\n " ++ BS.unpack k ++ ": " ++ BS.unpack v) "" (headers r) ++ "\n" ++ BS.unpack (body r) ++"\n}"
...
handleAccept :: Handle -> BS.ByteString -> IO Request
...
runServer (wch, rch, state, auth) = withSocketsDo $ do
  ...
  forever $ do (handle, hostname, port) <- accept sock               
               request <- handleAccept handle (BS.pack hostname)
               print request
